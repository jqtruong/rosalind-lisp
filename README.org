...
**** Sample
     ...
     least 1 Aa Bb organism, i.e. P_Ab * P_Bb.
...
**** Probability of "At Least Once"
     ...
     Pr_nope in our case is 3/4 to the 4^th (4 children)
     power, and the probability of at least one AaBb is:
...
**** Probability of "At Least N"
     ...
     only one is AaBb is, given:

     - Pr_notAaBb: 3/4
     - Pr_AaBb: 1/4

     Pr_1AaBb = 3/4^3*1/4 = 0.1055

     So "At Least 2" would be the probability for 2, 3, or 4
     out of 4 children, i.e. 1 minus the probability that 0
     or just 1 child are AaBb:

     1 - Pr_nope - Pr_1AaBb = 1 - 0.3164 - 0.1055 = 0.5781

     Based on [[https://math.stackexchange.com/questions/1477463/the-probability-of-at-least-18-successes-in-20-trials-binomial-distribution][this]], we need to take the factorial of the
     number of children multiplied by each probabilities,
     divided by the factorials of successes and failures.

     1 - (4!⋅Pr_nope)/(0!4!) - (4!⋅Pr_1AaBb)/(1!3!) = 0.2617

     why? [[Binomial Distribution][Binomial Distribution]].
**** Binomial Distribution
     Given a number of items, =n=, and a number of different
     items in =n='s set, =k=, the Binomial Coefficient: 
     ...
     There are 3! permutations: 6. Now supposed that c = b,
     essentially change this series from 3 differnt items to
     2, then rows 1 and 2, 3 and 5, 4 and 6 are the same,
     giving only 3 permutations. Using the formula above, we
     ...
     For the distruction part, the denominator depends on
     how many of each of the different items there is, like
     a derivative. For the problem at hand, there are 2^k
     items, 2 differents items (3/4 and 1/4, the complement
     probabilities for AaBb children), so let's start with
     the sample of k = 2 and N = 1. We still need to know
     Pr_nope, which is 3/4^4, such that none of the 4 children
     in the second generation are AaBb, which also give 0
     children are AaBb, giving:

     Pr_nope = 4! × (3/4^4 × 1/4^0) ÷ (4! × 0!)
...
** [[http://rosalind.info/problems/mprt/][Motif Implies Function]]
   ...
   Protein domain is an interval of amino acids that can
   evolve and function independently.

   Example functions of protein domains: 
   - binding the protein to DNA,
   - creating or breaking specific chemical bonds

   Proteins can be part of multiple domains.
